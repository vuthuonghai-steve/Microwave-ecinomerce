  ...
  <div class="p-4">
    ...
    <div class="mt-3 grid grid-cols-2 gap-2">
      <form method="post" action="{{ route('cart.add') }}">
        ...
        <button class="w-full bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded" onclick="event.stopPropagation();">Giỏ hàng</button>
      </form>
      <form method="post" action="{{ route('wishlist.add') }}">
        @csrf
        <input type="hidden" name="product_id" value="{{ $product->id }}" />
        <button class="w-full bg-pink-600 hover:bg-pink-700 text-white px-3 py-2 rounded" onclick="event.stopPropagation();">Yêu thích</button>
      </form>
    </div>
  </div>
</a>
